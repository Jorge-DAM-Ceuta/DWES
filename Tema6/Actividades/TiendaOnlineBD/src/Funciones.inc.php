...
function registrarUsuario($username, $password){
    ...
    $verificarUsuario = $conexionBD->query("SELECT COUNT(*) FROM usuario WHERE username = '$username'");
    $fila = $verificarUsuario->fetch_assoc();
    $usernameExistente = $fila['COUNT(*)'];
    ...
    if ($usernameExistente > 0) {
        ...
        $resultado = $conexionBD->query("INSERT INTO usuario (username, password, role) VALUES ('$username', '$passwordHash', 'Client');");

        //Mostrar comprobación.
        if($resultado == false){
            echo "<h2>No se ha podido registrar el usuario en este momento, intentalo de nuevo.</h2>";
        }else{
